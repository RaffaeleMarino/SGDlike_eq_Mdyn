...
void oneMCS(double temperature) {
  ...
  for (i = 0; i < N; i++) {
    ...
    if (deltaUnsat <= 0 || deltaUnsat < -temperature * log(FRANDOM)) { //to speedup, insert a look up table
      color[i] = newColor;
      numUnsat += deltaUnsat;
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 5) {
    fprintf(stderr, "usage: %s <N> <c> <Temp> <nIter> \n", argv[0]);
    exit(EXIT_FAILURE);
  }
  N = atoi(argv[1]);
  c = atof(argv[2]);
  ...
  NoverQ = (int)(N/Q);
  M = (int)(0.5 * c * N + 0.5);
  printf("# Q = %i   N = %i   M = %i   c = %f   temp = %f   seed = %ld\n",
	 Q, N, M, c, temp, seed);
